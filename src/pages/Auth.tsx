...
const Auth = () => {
  ...
  const [showLockoutModal, setShowLockoutModal] = useState<{ blockedUntil: Date } | null>(null);
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (limitData && !limitData.allowed) {
        setShowLockoutModal({ blockedUntil: new Date(limitData.blocked_until) });
        setIsLoading(false);
        return;
      }
  ...
  const SignInForm = () => {
    ...
    return (
      <motion.form key="signin" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} onSubmit={handleSignIn} className="space-y-6">
        ...
        <button type="button" onClick={handleGoogleSignIn} disabled={isLoading} className="w-full bg-background border border-input hover:bg-muted font-medium py-3 rounded-xl flex items-center justify-center gap-3 transition-colors">
        ...
        {isFeatureEnabled('guest_checkout') && (
          <button type="button" onClick={() => navigate('/')} className="w-full bg-secondary text-secondary-foreground font-bold py-3 rounded-xl hover:bg-secondary/80 transition-colors">
            Continue as Guest
          </button>
        )}
      </motion.form>
        ...
        {showLockoutModal && (
          ...
          >
            <motion.div
              ...
              animate={{ scale: 1, y: 0 }}
              className="bg-zinc-900 border border-red-500/30 rounded-3xl p-8 max-w-md w-full text-center shadow-2xl shadow-red-500/20"
            >
              <div className="w-24 h-24 mx-auto mb-6 bg-red-500/10 rounded-full flex items-center justify-center animate-pulse">
                <Shield size={48} className="text-red-500" />
              </div>

              <h2 className="text-3xl font-black text-white mb-2 tracking-tight">Access Suspended</h2>
              <p className="text-zinc-400 mb-8 leading-relaxed">
                Our active defense system detected multiple failed login attempts. To protect your account, we have temporarily locked access from this device.
              </p>

              <div className="bg-black/40 rounded-2xl p-6 border border-white/5 mb-8">
                <p className="text-xs text-zinc-500 uppercase tracking-widest font-bold mb-2">Try again in</p>
                <CountdownTimer targetDate={showLockoutModal.blockedUntil} />
              </div>

              <div className="flex items-center justify-center gap-2 text-xs text-zinc-500">
                <Lock size={12} />
                <span>Security Enforcement ID: {Math.random().toString(36).substring(7).toUpperCase()}</span>
              </div>
            </motion.div>
          </motion.div>
        )}
...
const CountdownTimer = ({ targetDate }: { targetDate: Date }) => {
  ...
  useEffect(() => {
    const update = () => {
      ...
      if (diff <= 0) {
        ...
        // Optionally auto-reload or clear modal here
      } else {
  ...
  return <p className="text-4xl font-mono font-bold text-red-500">{timeLeft}</p>;
};
